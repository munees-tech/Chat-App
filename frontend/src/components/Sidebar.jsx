...
const Sidebar = () => {
  ...
  return (
    <aside className="h-full w-20 sm:w-28 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200">
      ...
      {/* User List */}
      <div className="overflow-y-auto w-full py-3 px-2 sm:px-3">
        {filteredUsers.map((user) => (
          <button
            key={user._id}
            onClick={() => setSelectedUser(user)}
            className={`w-full p-2 sm:p-3 flex items-center gap-3 hover:bg-base-300 transition-colors rounded-lg ${
